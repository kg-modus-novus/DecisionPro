...
export async function runRefresh() {
  ...
  const sources = []; const federalSources = []; const metrics = []; const gaps = []; const datasets = [];
  try {
  ...
  const payload = { schema: 'decisionpro/fl-operational-sources/v1', generatedAt: new Date().toISOString(), loadClass: LOAD_CLASS, productState: 'FL', publisherPolicy: { sourceUri: `${BASE}/robots.txt`, contentHash: sha256(robots.bytes), contentSignal: 'search=yes, ai-train=no, use=reference', userAgent: USER_AGENT, interpretation: 'Reference use with attribution; no model training, raw-file redistribution, or source-of-record replacement. Technical policy reading, not legal clearance.' }, completionBoundary: 'Permitted AHCA exports and the official CMS MCPAR Florida slice are hydrated as governed public aggregates. Export-disabled workbooks and unreconciled parameter-driven series remain explicit Gaps.', sourceCount: sources.length, federalSourceCount: federalSources.length, datasetCount: datasets.length, metricCount: metrics.length, sources, federalSources, datasets, metrics, gaps };
