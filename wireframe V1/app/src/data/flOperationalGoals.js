...
function action({ id, title, priority, summary, owner, authority, how, impact, time, cost, savings, measures, guardrail, opportunity, roomLink }) {
  return { id, kind: 'action', title, reviewPriority: priority, implementationPriority: priority === 1 ? 'Review first' : 'Review next', summary, owner, authority, expectedImpact: impact, timeHorizon: time, how, estimatedCost: cost, estimatedSavings: savings, prerequisites: ['Confirm source period and definition', 'Name an accountable reviewer', 'Record the baseline and stopping rule'], successMeasures: measures, guardrail, opportunity, roomLink };
}

function goal({ id, label, objective, lead, leadLabel, title, question, confidence, lenses, inputs, transforms, actions, evidencePanels }) {
  return { id, label, objective, leadValue: lead.displayValue, leadLabel, reviewPriority: 1, readiness: 'Public evidence hydrated; validate before implementation', cases: [{ id: `${id}-case`, title, question, confidence, impactLenses: lenses, inputs, transformations: transforms, actions, ...(evidencePanels ? { evidencePanels } : {}) }] };
}
...
export const FL_OPERATIONAL_GOALS = [
  ...
  goal({
    ...
    evidencePanels: [{ id: 'fl-mcpar-plan-period', kind: 'mcpar-plan-period', state: 'FL', mode: 'full' }],
