...
**Status:** Director-authorized 2026-08-31; execution delegated to an autonomous agent run per `docs/planning/ofr-kickoff-prompt.md`. **OFR-00 through OFR-09 complete — all ten packages implemented and gate-green as of 2026-08-31.** The 2026-09-01 follow-up is also implemented and warehouse/build/harness/isolated-rendered accepted: recipient-level single-stream rows, complete action playbooks, direct recommendation-to-evidence controls, prime→sub-recipient and owner→facility relationship graphs, and the refreshed v2 ownership payload are available in both KY and FL. `GAP-CHAIN-QUALITY-ROLLUP` is resolved. A focused Vitest invocation still deadlocks before collection in this checkout and the warehouse `tsc --noEmit` attempt likewise produced no bounded result; those tooling checks are recorded as unavailable rather than green, while direct assertions, live CMS reconciliation, production builds, the canonical harness, and isolated-rendered journeys pass. See `docs/planning/ofr-completion-report.md` "2026-09-01 usability and relationship-graph follow-up." OFR-08 was built as a dedicated new room component rather than retrofitting the Kentucky-only ALP cube engine or the Florida bespoke-room pattern. See "OFR-09 detail" in the completion report for the original acceptance sweep and consolidated Gap registry. A security incident (SAM.gov key briefly exposed in the conversation transcript via an unredacted error message, never persisted to any repo/database/export artifact) occurred and was resolved during OFR-02, and a critical silent state-scoping bug (a CMS API filter that failed open and landed all-US data instead of KY+FL only, never reaching any export) occurred and was resolved during OFR-04 — see `docs/planning/ofr-completion-report.md` "Security incident (OFR-02)" and "OFR-04 detail" sections. **Plan amended 2026-08-31 after OFR-01:** SAM.gov key provisioned — `SAM_ENTITY` upgraded to hybrid-preferred and OFR-02's seed order revised accordingly; SAM key-expiry monitoring (BW admin dashboard banner) implemented directly by the Director's session — re-read the `SAM_ENTITY` row, OFR-02 row, and credential gate before resuming.
**2026-09-02 follow-on:** the depot-inference build (`docs/planning/operational-briefing-depot-inference-plan.md`) lifted the OFR export caps, added the prime-award, assistance-listing, provider-context, and depot-link join keys, added award class and renewal history to the award grain, added the MCPAR plan-period accountability record, and put an Operational briefing strip above the goal tiles on both state pages. Bundles regenerate with `ofr-depot-export`.
**App ID:** `decisionpro`
